...
public class FeatureTest {
  ...
  private void drawSvg(String name, List<Feature> featureList, Domain domain) {
    if (domain == null) {
      domain = ListFunc.reduce(featureList, new Domain(), (geoJson, dom) -> Domain.union (geoJson.domain, dom));
  ...
  public void testSimple() {
    var geoJsonList = Feature.readGeoJsonFile(Paths.get("data", "simple.json").toString());
  ...
  public void testSimpleWithHole() {
    var geoJsonList = Feature.readGeoJsonFile(Paths.get("data", "simple_with_hole.json").toString());
  ...
  public void testTexas() {
    var geoJsonList = Feature.readGeoJsonFile(Paths.get("data", "texas.json.gz").toString());
  ...
  @Test
  public void testUsa() {
    var geoJsonList = Feature.readGeoJsonFile(Paths.get("data", "USA-ADM0.geojson.gz").toString());
    drawSvg ("usa", geoJsonList, new Domain(-180, -60, 15, 75));
  }

  @Test
  public void testStates() {
    var geoJsonList = Feature.readGeoJsonFile(Paths.get("data", "USA-ADM1.geojson.gz").toString());
    drawSvg ("states", geoJsonList, new Domain(-180, -60, 15, 75));
  }

  @Test
  public void testBadData() {
    var geoJsonList = Feature.readGeoJsonFile(Paths.get("data", "bad_data.json").toString());
