...
public class SampledFunction extends Function {
  ...
  public static SampledFunction fromDatabase (Rows rows, BoundaryBehavior bbX, BoundaryBehavior bbY) {
    ...
    SampledFunction function = new SampledFunction (rows.domain, rows.interval, bbX, bbY);
  ...
  public static SampledFunction fromFunction (Function f, Domain domain, Tuple interval, BoundaryBehavior bbX, BoundaryBehavior bbY) {
  ...
  public static SampledFunction fromFunction (Function f, Domain domain, Tuple interval, BoundaryBehavior bb) {
    return fromFunction (f, domain, interval, bb, bb);
  }
  ...
  public Tuple refineSampleLocation (Tuple xy, double targetValue, int steps) {
    for (int i = 0; i < steps; ++i) {
      ...
      double delta = (at.f - targetValue) * 0.5;
  ...
  public Crossing findCrossing(TupleFunctionAt tfa, double xa, double xb, double targetValue) {
    ...
    // repeatedly bisect the search region until the two boundary points converge. this probably is
    // overkill (numerically), but we put a clamp on the number of iterations to prevent running out
    // of precision. the algorithm adds one bit of precision at each step, and we calculated the
    // number of bits based on our numerical precision goals (the worst case for 1.0e-10 precision
    // would be 33 iterations (log base 2), so not that much...).
    double lastX = 0.0 / 0.0;
    double nextX = (a.x + b.x) * 0.5;
    for (int i = 0; (i < Numerics.BITS_OF_PRECISION) && !Numerics.similar (lastX, nextX); ++i) {
      // get the midpoint of the domain
      Crossing c = at (tfa, nextX, targetValue);

      // shrink the domain to the midpoint for whichever point is on the same side of the target
