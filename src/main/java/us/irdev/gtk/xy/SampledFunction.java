...
import us.irdev.gtk.xy.bb.BoundaryBehavior;
...
public class SampledFunction {
  ...
  public SampledFunction (Domain domain, Tuple interval, BoundaryBehavior bbX, BoundaryBehavior bbY) {
    ...
    assert (Math.abs(((dimX - 1) * interval.x) - size.x) < TOLERANCE);
    assert (Math.abs(((dimY - 1) * interval.y) - size.y) < TOLERANCE);
  ...
  public At f (Tuple xy) {
    ...
    Tuple cxy = new Tuple (bbX.condition(xy.x, domain.min.x, domain.max.x), bbY.condition(xy.y, domain.min.y, domain.max.y));
    ...
    //   |    |      |
    //   A--- O -----B
    double m = lerp (a, c, yInterpolant);
    double n = lerp (b, d, yInterpolant);
    double o = lerp (a, b, xInterpolant);
    double p = lerp (c, d, xInterpolant);
    double r = lerp (m, n, xInterpolant);
      ...
      for (int ix = 0, ixEnd = (int) end.x; ix < ixEnd; ix++) {
        ...
        switch (code) {
          ...
          case 1: case 14: {
            ...
            Tuple left = new Tuple (x, where(targetValue, y, a, y1, c));
            Tuple bottom = new Tuple (where(targetValue, x, a, x1, b), y);
            output.add (new Segment (left, bottom));
          } break;
          case 2: case 13: {
            // bottom right corner is different, bottom and right have endpoints
            Tuple right = new Tuple (x1, where(targetValue, y, b, y1, d));
            Tuple bottom = new Tuple (where(targetValue, x, a, x1, b), y);
          ...
          case 4: case 11: {
            ...
            Tuple left = new Tuple (x, where(targetValue, y, a, y1, c));
            Tuple top = new Tuple (where(targetValue, x, c, x1, d), y1);
